  ...
  <div class="hlwidc-products-page">
    ...
    <div v-if="continents && continents.length > 0" class="hlwidc-continents-section">
      <div v-if="isMobile" class="hlwidc-mobile-select-wrapper">
        <el-select v-model="selectedContinent" placeholder="请选择销售大洲" size="large" class="hlwidc-mobile-select"
          @change="selectContinent" value-key="name">
          <el-option v-for="continent in continents" :key="continent.name"
            :label="`${continent.name} (${continent.countries?.length || 0} 个地区)`" :value="continent" />
        </el-select>
      </div>
      <div v-else class="hlwidc-continents-buttons">
        <button v-for="continent in continents" :key="continent.name" @click="selectContinent(continent)"
    ...
    <!-- 国家选择 -->
    <div v-if="selectedContinent && selectedContinentCountries && selectedContinentCountries.length > 0"
      class="hlwidc-countries-section">
      <div v-if="isMobile" class="hlwidc-mobile-select-wrapper">
        <el-select v-model="selectedCountry" placeholder="请选择国家/地区" size="large" class="hlwidc-mobile-select"
          @change="selectCountry" value-key="flag">
          <el-option v-for="country in selectedContinentCountries" :key="country.flag" :label="country.name"
            :value="country" />
        </el-select>
      </div>
      <div v-else class="hlwidc-countries-buttons">
        <button v-for="country in selectedContinentCountries" :key="country.flag" @click="selectCountry(country)"
    ...
    <!-- 省份选择 -->
    <div v-if="selectedCountry && selectedCountryProvinces && selectedCountryProvinces.length > 1"
      class="hlwidc-provinces-section">
      <div v-if="isMobile" class="hlwidc-mobile-select-wrapper">
        <el-select v-model="selectedProvince" placeholder="请选择省份" size="large" class="hlwidc-mobile-select"
          @change="selectProvince" value-key="flag">
          <el-option v-for="province in selectedCountryProvinces" :key="province.flag"
            :label="`${province.name} (${province.zones?.length || 0} 个可用区)`" :value="province" />
        </el-select>
      </div>
      <div v-else class="hlwidc-provinces-buttons">
        <button v-for="province in selectedCountryProvinces" :key="province.flag" @click="selectProvince(province)"
    ...
    <!-- 可用区选择 -->
    <div v-if="selectedProvince && selectedProvinceZones && selectedProvinceZones.length > 0"
      class="hlwidc-zones-section">
      <div v-if="isMobile" class="hlwidc-mobile-select-wrapper">
        <el-select v-model="selectedZone" placeholder="请选择可用区" size="large" class="hlwidc-mobile-select"
          @change="selectZone" value-key="id">
          <el-option v-for="zone in selectedProvinceZones" :key="zone.id"
            :label="`${zone.name} (${zone.city}) ${zone.area ? `(${zone.area})` : ''}`" :value="zone" />
        </el-select>
      </div>
      <div v-else class="hlwidc-zones-buttons">
        <button v-for="zone in selectedProvinceZones" :key="zone.id" @click="selectZone(zone)"
    ...
    <template v-if="selectedContinent">
      ...
      <el-table v-if="products && products.length > 0" v-loading="loading" :data="products"
        class="hlwidc-products-table" stripe>
        ...
        <el-table-column label="系统盘" min-width="120">
          <template #default="scope">
        ...
        <el-table-column label="数据盘" min-width="120">
          <template #default="scope">
        ...
        <el-table-column label="Windows" min-width="100">
          <template #default="scope">
            <span v-if="scope.row.noWindows === 1" class="hlwidc-windows-tag not-supported">不支持</span>
            <span v-else class="hlwidc-windows-tag supported">支持</span>
          </template>
        </el-table-column>
    ...
    <!-- 订购弹窗 -->
    <OrderModal :visible="showOrderModal" :product="selectedProduct" :currency-symbol="getCurrencySymbol(currencyUnit)"
      :format-price="formatPrice" :continent="selectedContinent" :country="selectedCountry" :province="selectedProvince"
      :zone="selectedZone" @close="closeOrderModal" @confirm="handleOrderConfirm" v-if="selectedProduct" />
  </div>
</template>
...
/* 移动端下拉选择器样式 */
.hlwidc-mobile-select-wrapper {
  margin-bottom: 20px;
}

.hlwidc-mobile-select {
  width: 100%;
}
